...
bool FurnaceState::canSmelt() {
    if (burningContainer.isEmpty()){
        return false;
    } else if ((!resultContainer.isEmpty())&&((resultContainer.getCurrentItem()!=instantiator->getItemFromNumber(getResultantItemNumber(burningContainer.getCurrentItem()))))){                     
        return false;
    } else if ((fuelContainer.isEmpty())&&(fuel == 0)) {
        return false;
    } else{
        return true; 
    }
...
void FurnaceState::addSmeltableItems() {
    this->smeltableItems = {2, 31, 32, 33, 38, 43, 45, 53};
}
