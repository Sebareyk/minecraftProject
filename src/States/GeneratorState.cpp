...
void GeneratorState::createLootTables() {
     ...
     */
    this->lootTables.push_back({30,30,30,30,30,30,30,30,30,30,33,33,33,33,32,32,32,31,31,41,41,66,66,36,});  //Pickaxe loot table
    this->lootTables.push_back({1 ,1 ,1 ,1 ,1 ,1 ,1 ,1 ,39,39,39,39,39,39,38,38,38,38,38,8}); // Shovel loot table
    this->lootTables.push_back({43,43,43,43,43,45,45,45,45,45,47,47,47,47,48,48,48,63,63,63,63,49,49,49}); // Sword loot table
    this->lootTables.push_back({4 ,4 ,4 ,4 ,4 ,4 ,4 ,4 ,4 ,4 ,4 ,4 ,3 ,3 ,3 ,3 ,3 ,2 ,2 ,2 }); // Axe loot table
    this->lootTables.push_back({51,51,51,51,51,51,51,51,53,53,53,53,53,53,55,55,55,55,55,68,68,68,37}); // Hoe loot table
}
...
void GeneratorState::generateItem() {
    ...
    else if (toolName.find("Ender")!= string::npos) toolTier = 0.000001; // The Ender Tools should now produce the best possible item.

    // These following lines increase a weapon's Tool Tier if it has the enchantment. By subtracting you increase the odds of it being a good item.
    if (toolContainer.getCurrentItem().getEnchantments().find("Efficiency") != string::npos) toolTier -= 2; // Efficiency
    else if (toolContainer.getCurrentItem().getEnchantments().find("Sharpness") != string::npos) toolTier -= 1; // Sharpness
    else if (toolContainer.getCurrentItem().getEnchantments().find("Looting") != string::npos) toolTier -= 5; // Looting
    else if (toolContainer.getCurrentItem().getEnchantments().find("Fire Aspect") != string::npos) toolTier += 2; // Fire Aspect (makes it worse) 
    // I would've made it so that "Fire Aspect" would give the food items cooked, but I was running out of time :'(  
